...
def load_config() -> dict:
    ...
    # pentru v3, default la locatii_cu_categorii.json
...
def load_places(path: str) -> List[Dict[str, Any]]:
    ...
    Accepts either:
    - dict with key 'locations' (formatul locatii_cu_categorii.json)
    - list of places (formatul vechi, simplu)
    """
...
def detect_language(text: str) -> str:
    ...
    ro_hints = {
        "unde", "ce", "imi", "imi", "vreau", "pot", "nu",
        "loc", "locuri", "locatii", "oras", "mancare", "cafea",
        "cafenea", "cafenele", "pranz", "cina", "prieteni",
        "gasca", "ieftin", "scump",
    }

    en_hints = {
        "what", "where", "which", "places", "place", "location",
        "locations", "coffee", "brunch", "breakfast", "lunch",
        "dinner", "cheap", "expensive", "friends", "date",
        "cozy", "burger", "pizza", "vegan", "pub", "bar",
        "remote", "work",
    }
...
CITY_TOKENS = {
    ...
    "sibiu", "constanta", "oradea",
    "galati", "craiova", "ploiesti",
    "targu", "targumures", "mures",
    "alba", "iulia",
}
...
def is_all_places_question(query: str) -> bool:
    ...
    if ("places" in tokens or "locations" in tokens) and (
        "all" in tokens or "list" in tokens
...
def is_cafes_list_question(query: str) -> bool:
    ...
    cafe_tokens = {
        "cafenea", "cafenele", "cafea", "cafe",
        "coffee", "coffeeshop", "coffeeshops", "shop", "shops"
    }
...
def handle_list_restaurants(places: List[Dict[str, Any]], lang: str) -> str:
    ...
    if lang == "en":
        ...
        lines.append("Uite toate restaurantele și locurile de mâncare pe care le am în aplicație, grupate pe oraș:")
...
def generate_vibe_for_place(
    ...
    # aici folosim temperatura 0.5 pentru un vibe stabil dar totuși plăcut
    return call_groq(client, model, [system_msg, user_msg], max_tokens=220, temperature=0.5)
...
def chatbot_loop(client: Groq, model: str, places: List[Dict[str, Any]]) -> None:
    ...
    while True:
        ...
        history_lines = []
        for msg in history[-6:]:  # last few turns only, ca să nu umflăm prompt-ul
...
def main() -> None:
    ...
    print("=== Thecon Hackathon AI Debug (Chat_Bot_Groq_v3.py) ===")
